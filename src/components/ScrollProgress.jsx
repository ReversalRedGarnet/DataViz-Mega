...
export default function ScrollProgress() {
  ...
  useLayoutEffect(() => {
    ...
    if (!el) return
    // getBoundingClientRect().width is fractional; el.offsetWidth would
    // round it to the nearest integer, which can round *up* past the
    // wrapper's true width on any layout that lands on a fractional
    // pixel (common with flex/percentage widths). Flooring instead of
    // rounding guarantees this can never end up wider than the space
    // it's actually been given, at the cost of at most one wasted
    // sub-pixel of width -- an intermittent, hard-to-reproduce
    // one-pixel overflow (seen roughly 60% of runs in testing) beat an
    // always-safe floor every time.
